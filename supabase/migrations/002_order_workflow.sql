...
-- Public can read their order by access token (via service role API only — no direct public SELECT)
-- Public can update proof approval via API route with token validation
